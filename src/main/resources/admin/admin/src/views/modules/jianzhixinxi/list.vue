<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0px 0px 20px","overflow":"hidden","color":"#666","background":"#f4f8fb","fontSize":"14px","position":"relative"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px 20px 20px 0","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","background":"none","width":"auto","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px","borderColor":"#4285f430","borderRadius":"0px","textAlign":"center","flexWrap":"wrap","background":"#4285f410","flexDirection":"row","borderWidth":"1px 1px 0","display":"flex","width":"100%","fontSize":"inherit","borderStyle":"solid"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">企业名称</label>
						<el-input v-model="searchForm.qiyemingcheng" placeholder="企业名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">企业规模</label>
						<el-input v-model="searchForm.qiyeguimo" placeholder="企业规模" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' class="select" label="学科门类" prop="xuekemenlei">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">学科门类</label>
						<el-select clearable v-model="searchForm.xuekemenlei" placeholder="请选择学科门类" >
							<el-option v-for="(item,index) in xuekemenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0px 20px 10px","margin":"0","borderColor":"#4285f430","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","flexWrap":"wrap","background":"#4285f410","borderWidth":"0 1px 1px","width":"100%","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button class="add" v-if="isAuth('jianzhixinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						添加信息
					</el-button>
					<el-button class="del" v-if="isAuth('jianzhixinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #e1e8ee","width":"calc(100% - 20px)","padding":"0","margin":"0","borderRadius":"0","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="学科门类">
						<template slot-scope="scope">
							{{scope.row.xuekemenlei}}
						</template>
					</el-table-column>
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="zhaopinfengmian" width="200" label="招聘封面">
						<template slot-scope="scope">
							<div v-if="scope.row.zhaopinfengmian">
								<img v-if="scope.row.zhaopinfengmian.substring(0,4)=='http'" :src="scope.row.zhaopinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.zhaopinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jianzhixinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('jianzhixinxi','应聘')" @click="yingpinxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								应聘
							</el-button>
							<el-button class="btn8" v-if="isAuth('jianzhixinxi','投诉')" @click="tousuxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								投诉
							</el-button>
							<el-button class="edit" v-if=" isAuth('jianzhixinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jianzhixinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<tousuxinxi-cross-add-or-update v-if="tousuxinxiCrossAddOrUpdateFlag" :parent="this" ref="tousuxinxiCrossaddOrUpdate"></tousuxinxi-cross-add-or-update>
...
import tousuxinxiCrossAddOrUpdate from "../tousuxinxi/add-or-update";
	export default {
		data() {
			return {
				xuekemenleiOptions: [],
				...
				tousuxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yingpinxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yingpinxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jianzhixinxi');
			...
			tousuxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.tousuxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jianzhixinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tousuxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tousuxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
          this.$http({
            url: `option/xuekemenlei/xuekemenlei`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.xuekemenleiOptions = data.data;
            } else {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.qiyeguimo!='' && this.searchForm.qiyeguimo!=undefined){
            params['qiyeguimo'] = '%' + this.searchForm.qiyeguimo + '%'
          }
           if(this.searchForm.xuekemenlei!='' && this.searchForm.xuekemenlei!=undefined){
            params['xuekemenlei'] = this.searchForm.xuekemenlei
          }
	...
	jianzhixinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'jianzhixinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
