...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知标题" prop="tongzhibiaoti">
            <el-input v-model="ruleForm.tongzhibiaoti" 
                placeholder="通知标题" clearable :disabled=" false  ||ro.tongzhibiaoti"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知时间" prop="tongzhishijian">
              <el-date-picker
				  :disabled=" false  ||ro.tongzhishijian"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="通知状态" prop="tongzhizhuangtai">
            <el-select v-model="ruleForm.tongzhizhuangtai" placeholder="请选择通知状态" :disabled=" false  ||ro.tongzhizhuangtai" >
              <el-option
                  v-for="(item,index) in tongzhizhuangtaiOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知详情" prop="tongzhixiangqing">
            <editor 
                ...
                v-model="ruleForm.tongzhixiangqing" 
                class="editor" 
                action="file/upload">
            </editor>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='tongzhizhuangtai'){
              this.ruleForm.tongzhizhuangtai = obj[o];
              this.ro.tongzhizhuangtai = true;
              continue;
            }
            if(o=='tongzhixiangqing'){
              this.ruleForm.tongzhixiangqing = obj[o];
              this.ro.tongzhixiangqing = true;
              continue;
            }
          }
        }else if(type=='edit'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`mianshixinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('mianshixinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`mianshixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
