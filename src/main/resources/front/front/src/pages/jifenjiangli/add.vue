...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="兼职时长" prop="jianzhishizhang">
            <el-input v-model.number="ruleForm.jianzhishizhang" 
                placeholder="兼职时长" clearable :disabled=" false  ||ro.jianzhishizhang"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="效率高" prop="xiaolvgao">
            <el-input v-model="ruleForm.xiaolvgao" 
                placeholder="效率高" clearable :disabled=" false  ||ro.xiaolvgao"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="好评" prop="haoping">
            <el-input v-model="ruleForm.haoping" 
                placeholder="好评" clearable :disabled=" false  ||ro.haoping"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="奖励积分" prop="jifen">
            <el-input v-model.number="ruleForm.jifen" 
                placeholder="奖励积分" clearable :disabled=" false  ||ro.jifen"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="奖励时间" prop="jianglishijian">
              <el-date-picker
				  :disabled=" false  ||ro.jianglishijian"
                  value-format="yyyy-MM-dd HH:mm:ss"
                  v-model="ruleForm.jianglishijian" 
                  type="datetime"
                  placeholder="奖励时间">
              </el-date-picker>
          </el-form-item>
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        ro:{
				...
				xuekemenlei : false,
				shouji : false,
				jianzhishizhang : false,
				xiaolvgao : false,
				haoping : false,
				touxiang : false,
				jifen : false,
				jianglishijian : false,
        },
        type: '',
    ...
    created() {
      ...
      this.ruleForm.jianglishijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jianzhishizhang'){
              this.ruleForm.jianzhishizhang = obj[o];
              this.ro.jianzhishizhang = true;
              continue;
            }
            if(o=='xiaolvgao'){
              this.ruleForm.xiaolvgao = obj[o];
              this.ro.xiaolvgao = true;
              continue;
            }
            if(o=='haoping'){
              this.ruleForm.haoping = obj[o];
              this.ro.haoping = true;
              continue;
            }
            if(o=='touxiang'){
            ...
            if(o=='jianglishijian'){
              this.ruleForm.jianglishijian = obj[o];
              this.ro.jianglishijian = true;
              continue;
            }
          }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jifenjiangli/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jifenjiangli/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(table+`/update`,obj).then(res => {});
								this.$http.post(`jifenjiangli/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
