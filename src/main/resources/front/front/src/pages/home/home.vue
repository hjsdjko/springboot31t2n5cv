...
<div class="home-preview" :style='{"margin":"0 auto","flexWrap":"wrap","flexDirection":"row","background":"url(http://codegen.caihongy.cn/20240117/40df7b0ed9d446dc9e3412e83abd813d.jpg) no-repeat center bottom,#d4eeff","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
	...
	<div id="animate_newsnews" class="news animate__animated" :style='{"padding":"40px 7%","margin":"0 auto","backgroundAttachment":"fixed","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","order":"4"}'>
		...
		<div class="title" :style='{"width":"100%","padding":"0","margin":"0 auto","textAlign":"center","background":"url(http://codegen.caihongy.cn/20240117/e2da3af842f44f9f86c61d0ff295414d.png) no-repeat center center / 35%","height":"60px"}'>
			<span :style='{"fontSize":"26px","lineHeight":"60px","color":"#454545","fontWeight":"600"}'>公告信息</span>
		</div>
...
<!-- 商品推荐 -->
<div id="animate_recommendjianzhixinxi" class="recommend animate__animated" :style='{"padding":"40px 7% 40px","margin":"0 auto","borderColor":"#ddd","flexWrap":"wrap","background":"none","borderWidth":"0px 0","display":"flex","width":"100%","position":"relative","borderStyle":"solid","order":"2"}'>
    ...
    <div class="title" :style='{"width":"100%","margin":"0 auto","textAlign":"center","background":"url(http://codegen.caihongy.cn/20240117/e2da3af842f44f9f86c61d0ff295414d.png) no-repeat center center / 35%","height":"60px"}'>
		<span :style='{"padding":"0px 0 0","fontSize":"26px","lineHeight":"60px","color":"#454545","fontWeight":"600","display":"block"}'>兼职信息推荐</span>
	</div>
		...
		<div v-if="jianzhixinxiRecommend.length" :style='{"padding":"0","margin":"10px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"3"}' class="list list17 index-pv1">
		  <div v-for="item,index in jianzhixinxiRecommend" :key="index"  @click="toDetail('jianzhixinxiDetail', item)" :style='{"cursor":"pointer","width":"calc(25% - 20px)","margin":"0 20px 20px 0","position":"relative","background":"none","height":"auto"}' class="item">
				<img class="image" v-if="preHttp(item.zhaopinfengmian)" :src="item.zhaopinfengmian.split(',')[0]" alt="" />
				<img class="image" v-else :src="baseUrl + (item.zhaopinfengmian?item.zhaopinfengmian.split(',')[0]:'')" alt="" />
			<div class="content">
		      <div :style='{"lineHeight":"24px","fontSize":"14px","color":"#333","display":"none"}'>{{item.jingyanyaoqiu}}</div>

		      <div class="info">
				<div :style='{"overflow":"hidden","whiteSpace":"hidden","color":"#333","textAlign":"left","width":"100%","lineHeight":"1.3","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"500"}' class="name ">{{item.gangweifenlei}}</div>
		        ...
		        <div :style='{"padding":"0","display":"none"}'>
		          ...
		          <span :style='{"color":"#fff","lineHeight":"1.5","fontSize":"12px"}'>{{item.qiyezhanghao}}</span>
		        </div>
		        <div :style='{"padding":"0","display":"none"}'>
	...
	<div @click="moreBtn('jianzhixinxi')" :style='{"border":"0px solid #eccc19","cursor":"pointer","padding":"0 20px","margin":"0","borderRadius":"20px","textAlign":"RIGHT","background":"none","display":"block","width":"100%","lineHeight":"32px"}'>
		<span :style='{"color":"#3389d7","fontSize":"22px"}'>MORE</span>
		<i :style='{"color":"#3389d7","fontSize":"22px","display":"inline-block"}' class="icon iconfont icon-jiantou25"></i>
	</div>
    ...
    methods: {
		...
		handleScroll() {
			let arr = [
				...
				{id:'animate_recommendjianzhixinxi',css:'animate__'},
      ...
      preHttp(str) {
		...
		getList() {
			let autoSortUrl = "";
			let data = {}
          autoSortUrl = "jianzhixinxi/autoSort";
          if(localStorage.getItem('frontToken')) {
              autoSortUrl = "jianzhixinxi/autoSort2";
          }
			data = {
				page: 1,
				limit: 8,
			...
			this.$http.get(autoSortUrl, {params: data}).then(res => {
				if (res.data.code == 0) {
					this.jianzhixinxiRecommend = res.data.data.list;
